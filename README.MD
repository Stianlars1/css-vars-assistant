...
| **Package-root CSS-entrypoint fallback** | When a package's Sass entrypoint is mixin-only (no concrete `--*` declarations), the resolver falls back to the `css` / `style` entrypoint declared in `package.json`, so design systems that emit tokens via a `@include core()` mixin still work. |
